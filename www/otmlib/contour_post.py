...
from otmlib.constants import STEEP_DEG
from otmlib.osm_areas import AreaIndex, count_ways, pbf_bbox
from otmlib.proc import check_cancelled, worker_count
from otmlib.progress import Progress
...
def postprocess_contour_pbfs(
    ...
    glacier_pbf: Path | None,
    dem_files: list[Path],
) -> None:
    """Tag contours in place. *glacier_pbf* is the glacier-only extract from the
    tile job — whole polygons, so an ice edge is never cut by a tile boundary."""
    if not contour_pbfs:
        return
    subset = glacier_pbf if glacier_pbf is not None and glacier_pbf.is_file() else None
    glaciers = AreaIndex.load(subset, "natural", "glacier", prefiltered=True) if subset else None
