...
# Something the glacier filter must drop, well clear of the ice.
_FOREST = [
    (42.4200, 43.3100),
    (42.4250, 43.3100),
    (42.4250, 43.3140),
...
def glacierScene(
    ...
    *forest*: add a wood polygon, to check that a filter keeps it out.
    """
    ...
    if crevasse == "inside":
        ...
        chunks.append(_way(11, c_refs, {"natural": "crevasse"}))
    if forest:
        f_nodes, f_refs = _ring_nodes(40, _FOREST)
        chunks.append(f_nodes)
        chunks.append(_way(12, f_refs, {"natural": "wood"}))
    chunks.append("</osm>")
    path.write_text("\n".join(chunks) + "\n", encoding="utf-8")
    return path
