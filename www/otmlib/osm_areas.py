...
from otmlib.constants import GLACIER_FILTER
...
def filtered_subset(pbf: Path, filters: Sequence[str], output: Path) -> Path:
    """Write the objects matching *filters* (osmium expressions) to *output*.
    ...
    # Write beside the target and rename: readers of a published subset (the Garmin
    # build) must never see a half-written file. The extension has to survive the
    # detour — osmium picks its output format from the file name.
    tmp = output.parent / f".{output.stem}.part{output.suffix}"
    tmp.unlink(missing_ok=True)
    try:
        run([osmium_bin, "tags-filter", str(pbf), *filters, "-o", str(tmp), "--overwrite"])
    except Exception:
        tmp.unlink(missing_ok=True)
        raise
    tmp.replace(output)
...
def glacier_subset(pbf: Path, output: Path) -> Path:
    """Glacier-only extract: ice, crevasses and moraines, whole polygons.

    Built once per region by the tile job. The Garmin contour tagging and the
    crevasse hatch read it instead of the full extract, which is orders of
    magnitude larger, and instead of the glacier tiles, which cut polygons.
    """
    return filtered_subset(pbf, GLACIER_FILTER, output)
...
def tagged_subset(pbf: Path, key: str, value: str) -> Path:
    ...
    tmp = Path(name)
    try:
        return filtered_subset(pbf, [f"nwr/{key}={value}"], tmp)
    except Exception:
        tmp.unlink(missing_ok=True)
        raise
