...
from otmlib.osm_areas import tagged_subset
...
def hatch_crevasses(
    crevasses: list[BaseGeometry],
    glaciers: list[tuple[BaseGeometry, str | None]],
) -> list[tuple[LineString, int]]:
    """Hatch ticks across every crevasse, perpendicular to its host glacier's flow.

    Takes geometries so the same cartography can be driven from an OSM extract
    (the Garmin build) or from decoded vector tiles (the contour service).
    """
    if not crevasses:
        log.info("Crevasse hatch: no natural=crevasse areas")
        return []
    hosts: list[tuple[BaseGeometry, float]] = []
    skipped_no_dir = 0
    for geom, raw_dir in glaciers:
        azimuth = parse_direction(raw_dir)
        if azimuth is None:
            skipped_no_dir += 1
            continue
        hosts.append((geom, azimuth))
    if not hosts:
        log.info(
    ...
    tree = STRtree([geom for geom, _ in hosts])
    lines: list[tuple[LineString, int]] = []
    skipped_no_host = 0
    for crevasse in crevasses:
        azimuth = _host_glacier(crevasse, hosts, tree)
        if azimuth is None:
            skipped_no_host += 1
            continue
...
def extract_crevasse_stripes(pbf: Path, *, prefiltered: bool = False) -> list[tuple[LineString, int]]:
    """Hatch ticks for every crevasse. With *prefiltered* the file is already the
    glacier-only extract, so the per-tag osmium pass is skipped."""
    crevasses = [geom for geom, _ in _load_areas(pbf, "natural", "crevasse", prefiltered=prefiltered)]
    glaciers = _load_areas(pbf, "natural", "glacier", prefiltered=prefiltered) if crevasses else []
    return hatch_crevasses(crevasses, glaciers)
...
def build_crevasse_stripes(pbf: Path, output: Path, *, prefiltered: bool = False) -> Path | None:
    lines = extract_crevasse_stripes(pbf, prefiltered=prefiltered)
    if not lines:
