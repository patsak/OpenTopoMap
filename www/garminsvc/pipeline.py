...
class BuildContext:
    ...
    mkgmap_jar: Path = field(default_factory=lambda: artifacts.jar_path(artifacts.MKGMAP, TOOLS_DIR))
...
def prepare_pbf_tiles(ctx: BuildContext) -> None:
    ...
    cmd = [
        "java",
        f"-Xmx{ctx.java_mem}",  # JVM heap (JAVA_MEM, 4g by default)
        "-jar",
        str(ctx.splitter_jar),
        # Prebuilt coastline: tiles are cut correctly along the sea
        f"--precomp-sea={ctx.sea_dir}",
        f"--output-dir={split}",
        f"--mapid={ctx.family_id_map}0001",
    ]
    if ctx.polygon_file and ctx.polygon_file.is_file():
        # Clip OSM to the bbox polygon instead of splitting the whole extract
...
def build_ridges(ctx: BuildContext) -> None:
    ...
    if src is None or not src.is_file():
        ...
        ctx.ridges_osm = None
        return
    from otmlib.ridges import build_ridges as _build

    log.info("Ridge lines (natural=ridge/arete): %s", src)
    out = ctx.data_dir / "ridges-hike" / "ridges.osm"
    ctx.ridges_osm = _build(src, out)
    if not ctx.ridges_osm:
        log.info("No ridge lines (no natural=ridge/arete ways in the extract)")
...
def build_main_map(ctx: BuildContext) -> None:
    ...
    cmd = [
        ...
        "-jar",
        str(ctx.mkgmap_jar),
        "-c",  # shared mkgmap options (gmapsupp, index, routing, …)
        str(OPTIONS_MAIN),
        f"--style-file={STYLE_DIR / 'opentopomap-hike'}",
        f"--precomp-sea={ctx.sea_dir}",
        f"--bounds={ctx.bounds_dir}",  # administrative boundaries / addresses
        f"--dem={ctx.hgt_dir}",
        f"--dem-dists={DEM_DISTS}",  # DEM step per zoom level, in metres
        "--show-profiles=1",  # elevation profiles on the device
        "--gmapi",  # .gmap directory for BaseCamp
        f"--family-id={ctx.family_id_map}",  # must match the TYP and the splitter mapid
    ...
    if ctx.dem_poly and ctx.dem_poly.is_file():
        # Clip the DEM to the same polygon the OSM data was clipped to
...
def build_contours_map(ctx: BuildContext) -> None:
    ...
    run(
        [
            ...
            "-jar",
            str(ctx.mkgmap_jar),
            "-c",  # contour options (no routing/index, unlike the main map)
            str(OPTIONS_CONTOURS),
            f"--style-file={STYLE_DIR / 'contours-hike'}",
            f"--family-id={ctx.family_id_contours}",
            f"--mapname={ctx.family_id_contours}0001",  # id of the main contour tile
            f"--overview-mapname={ctx.family_id_contours}0000",  # overview map
