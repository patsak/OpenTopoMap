...
def normalize_upload_name(filename: str) -> str:
    ...
    if not any(lower.endswith(suffix) for suffix in ALLOWED_SUFFIXES):
        raise UploadError("An .osm, .osm.pbf or .pbf file is required (.gz / .bz2 accepted)")
    return name
...
def bbox_from_pbf(pbf: Path) -> tuple[float, float, float, float]:
    ...
    if handler.n == 0:
        raise RuntimeError(f"The file carries no coordinates: {pbf.name}")
...
def to_pbf(src: Path, dest: Path) -> Path:
    ...
    if not dest.is_file() or dest.stat().st_size == 0:
        raise RuntimeError(f"osmium cat produced no PBF from {src.name}")
    return dest
