...
CONFIG_SOURCE = runner.CONFIG_REGION
# Only the zoom range is the config's own: the whole ladder, so the preview
# stays on screen while zooming out instead of vanishing below some floor. A
# bbox is small enough that its low zooms are a rounding error in the build -
# the tile count at z0-9 over one drawn rectangle is a handful.
MINZOOM = 0
MAXZOOM = 14
...
def write_config(styles: Path, work: Path) -> Path:
    ...
    config["settings"]["name"] = "OpenTopoMap (area preview)"
...
def _sync_inputs(cfg: Config, preview: previews.Preview, log_fn) -> list[Path]:
    ...
    log_fn(f"Regions: {', '.join(region.name for region in regions)}")
...
def build(preview_id: str, cfg: Config | None = None) -> None:
    ...
    try:
        ...
        west, south, east, north = preview.bbox
        log_fn("Cutting the area out…")
        area_pbf = extract_bbox(pbfs, west, south, east, north, work / "preview.osm.pbf")

        log_fn(f"tilemaker: zooms {MINZOOM}–{MAXZOOM}…")
        styles = runner.style_dir()
        runner.build(
            output=staging,
        ...
        shutil.rmtree(work, ignore_errors=True)
        # By age first, then by count: an expired preview is gone whether or
        # not it was among the newest KEEP_PREVIEWS.
        previews.expire(out_dir)
        previews.prune(KEEP_PREVIEWS, out_dir)


def recover(cfg: Config | None = None) -> None:
    """Sweep expired previews, then re-queue what a killed worker left behind
