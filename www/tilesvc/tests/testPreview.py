...
def worker(cfg, region, tmp_path):
    ...
    patches = [
        ...
        mock.patch.object(preview.runner, "style_dir", return_value=styles),
    ]
...
class TestExpiry:
    def testAnExpiredPreviewIsNotReusedButBuiltAgain(self, worker):
        first = previews.create(*BBOX)
        preview.build(first.preview_id, worker.cfg)
        makeExpired(first.preview_id)
        # The picker asks for the same rectangle: nothing to reuse now, so the
        # area goes through tilemaker a second time.
        assert previews.find_ready(*BBOX, previews_dir=worker.dir) is None
        second = previews.create(*BBOX)
        preview.build(second.preview_id, worker.cfg)
        assert len(worker.built) == 2

    def testABuildSweepsExpiredPreviewsOffTheDisk(self, worker):
        old = previews.create(1.0, 43.0, 1.5, 43.4)
        preview.build(old.preview_id, worker.cfg)
        makeExpired(old.preview_id)
        fresh = previews.create(*BBOX)
        preview.build(fresh.preview_id, worker.cfg)
        assert previews.get(old.preview_id) is None
        assert not (worker.dir / f"{old.preview_id}.pmtiles").exists()
        assert (worker.dir / f"{fresh.preview_id}.pmtiles").is_file()
...
class TestRecover:
    ...
    def testAnExpiredQueuedPreviewIsDroppedInsteadOfRebuilt(self, cfg):
        row = previews.create(*BBOX)
        makeExpired(row.preview_id)
        with mock.patch.object(preview.previewqueue, "enqueue") as enqueue:
            preview.recover(cfg)
        assert previews.get(row.preview_id) is None
        assert enqueue.call_args_list == []
