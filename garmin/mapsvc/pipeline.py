...
from mapsvc.proc import check_cancelled, run, worker_count
...
def build_crevasse_stripes(ctx: BuildContext) -> None:
    ...
    if not contour_pbfs:
        ...
        ctx.crevasse_osm = None
        return
    from mapsvc.crevasse import build_crevasse_stripes as _build
...
def _outside_bbox(pbf: Path, area: str) -> bool:
    from mapsvc.osm_areas import pbf_bbox
...
def build_contour_pbfs(ctx: BuildContext) -> None:
    from mapsvc.contour_terrain import contour_step
    ...
    from mapsvc.contour_post import postprocess_contour_pbfs
