...
-- hike: cover moraine on ice. geological=moraine is the
...
function process_water_polygons(way_area)
	...
	if mz < inf_zoom then
		...
		ZOrder(way_area)
		-- hike: a Genshtab sheet prints the water surface elevation at a lake,
		-- named or not, so an ele tag alone is enough to earn a label centroid.
		-- Not on ice: the glacier label is the name only, so an ele-only glacier
		-- centroid would be an invisible label.
		local ele_only = kind ~= "glacier" and tonumber(Find("ele")) ~= nil
		if Holds("name") or ele_only then
