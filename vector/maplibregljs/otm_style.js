...
//
// Load after maplibre-gl.js and maplibre-contour (mlcontour). otm_layers.json
// is plain JSON now (no global), so its consumer fetches and parses it itself
// and passes the result in as opts.layers.
...
//   layers      layer list, parsed from otm_layers.json (required)
...
function otmVectorStyle(options) {
	...
	if (!opts.layers) {
		throw new Error("otmVectorStyle: opts.layers is required");
	}
